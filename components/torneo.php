<li>
    <article id="info-torneo">
        ...
        <div <?php echo 'id="tournament-edition-div' . $torneo['idTorneo'] .'"' ?> style="display:none;">
            <form action="../handlers/editar_torneo_handler.php" id="editarTorneoForm" method="POST" onsubmit="return editarTorneo()">
                ...
                <input type="text" name="titulo" id="titulo" value="<?php echo $torneo['titulo'] ?>"/>
                <label for="fecha">Fecha del torneo</label>
                <input type="date" name="fecha" id="fecha" value="<?php echo $torneo['fecha'] ?>"required/>
                <label for="hora">Hora del torneo</label>
                <input type="time" name="hora" id="hora" value="<?php echo $torneo['hora'] ?>" step="300" required/>
                <button type="submit">
                    Guardar
                </button>
        ...
        <div id="inscripciones-actions">
            ...
            <div id="inscribirse" onclick="inscribirUsuario(<?php echo $_COOKIE['idUser'] . ',' . $idTorneo  ?>)">
                Inscribirse
            </div>
        </div>
    </article>
    <?php foreach ($inscripciones as $inscripcion): ?>
        <div class="inscripcion">
            ...
            <button onclick="borrarInscripcion(<?php echo $inscripcion['idUser'] . ', ' . $idTorneo; ?>)">Borrar</button>
            <?php endif; ?> 
        </div>
    <?php endforeach; ?>
    <?php if($_COOKIE['rol'] == 'admin'): ?>
        ...
        <div id="buscador-container">
            ...
            <div id="buscador">
                <input type="text" placeholder="Buscar..." onkeyup="buscarUsuarios(this.value, <?php echo $idTorneo; ?>)" id="buscador-usuarios" />
                <div id="searchResults-<?php echo $idTorneo; ?>"></div>
            </div>
        </div>
    <?php endif; ?> 
